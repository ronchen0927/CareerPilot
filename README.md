...
如需使用 AI 功能（評分、求職信、履歷改寫、Q&A 對話），在 `backend/.env` 加入：
...
| AI 功能 | OpenAI API（GPT）、pdfplumber（PDF 解析）、Playwright（URL 擷取）、StreamingResponse（串流回應） |
| 前端 | React 18 + TypeScript + Vite |
| 路由 | React Router v6 |
| 套件管理 | uv（後端）、npm（前端） |
| 資料儲存 | localStorage（書籤、看板、CV、對話記錄、使用者偏好）、alerts.json（後端提醒設定） |
| 測試 | pytest + httpx |
